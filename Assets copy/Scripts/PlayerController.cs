...
public class PlayerController : MonoBehaviour
{
    ...
    {
        //Input is left mouse button, 
        if (Input.GetMouseButtonDown(0) && ammo > 0)
        {

            Shoot();
            // Does not work properly, only shoots forward not in direction of mouse.
            //FIX THIS
            ...
            {
                targetPoint = ray.GetPoint(1000); //aims far into distance if nothing hit by mouse. 
            }
            // Calculate the direction from the player to the target point
            //Direction and Distance between two points (Player and Mouse point)
            Vector3 direction = (targetPoint - transform.position).normalized;
            GameObject bullet = Instantiate(projectilePrefab, transform.position + transform.forward, transform.rotation);
            Rigidbody bulletRb = bullet.GetComponent<Rigidbody>();
            //This will shoot bullets forward
            if (bulletRb != null)
            {
